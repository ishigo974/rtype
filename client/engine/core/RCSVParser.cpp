...
#include <iostream>
void    RCSVParser::parse(std::multimap<double, Spawn> &multipass)
{
...
void    RCSVParser::parseLine(std::multimap<double, Spawn> &multipass,
                              std::string& line)
{
    std::stringstream           ss;
    ...
    multipass.emplace(stod(tokens[0]),
                      Spawn(static_cast<MobType>(stoi(tokens[2])),
